...
4. **Prove or kill.** Evidence, not vibes. Three dead hypotheses: stop.
   `architect` re-maps the subsystem with you.
...
6. **Close.** Normal `review` and `commit`. Put the root cause in
...
- Diagnostics stay on the Supervisor. Delegate only once the fix is mechanical.
...
| Round | Action |
|---|---|
| 1–2 | Supervisor diagnoses, builder fixes |
