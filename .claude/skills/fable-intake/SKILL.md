---
name: fable-intake
description: Session-start and new-request ritual for the Fable orchestrator. Restores context (lessons, ledger, active plans), restates the goal, resolves ambiguity, and picks the working lane. Use at the start of a session or whenever a new request arrives.
...
- Read `docs/fable/LESSONS.md` and any `docs/fable/LEDGER*.md`.
...
Look in `docs/fable/plans/` for a file with `status: active`. If one exists,
...
| The request is | Lane |
|---|---|
| Trivial, a one-sentence diff | Do it inline, then `fable-commit` |
| A single well-specified task | Light scout pass → brief → dispatch |
| A feature or multi-step change | `fable-plan` |
| A failure with an unknown cause | `fable-debug` |
| UI work with a mockup involved | `fable-ui` |
