---
name: commit
description: Pre-commit gate and commit choreography for the Supervisor — checks green, message drafted via scribe, one verified increment committed. Use after every verified task.
...
3. The critic pass is done where `review` requires it.
...
- `scribe` drafts a Conventional Commit from the diff plus the brief:
